...
export default function RouteWorkshop({ data, onFinish }: { data: WorkshopData; onBack: () => void; onFinish: () => void }) {
  ...
  return <div className="workshop embedded-workshop">
    ...
    <section className="workshop-sheet">
      ...
      {task === 5 && <><h1>Заполни мини-кроссворд</h1><p className="task-intro">Впиши четыре слова, которые встречались в маршруте.</p>{data.crossword.map((clue, index) => <label className="cross-row" key={clue[0]}><span className={data.route === "Московские высотки" ? "key-fact" : undefined}>{index + 1}. {clue[0]}</span><input value={cross[index]} onChange={event => { const next = [...cross]; next[index] = event.target.value; setCross(next); }} placeholder={`${clue[1].length} букв`} /></label>)}<button className="primary" onClick={() => result(cross.map(clean).join("|") === data.crossword.map(item => clean(item[1])).join("|"), "Верно! Все слова заполнены.", "Есть неточность. Все ответы встречались в учебных экранах.")}>Проверить кроссворд</button></>}
