...
const workshopData: WorkshopData = {
  ...
  anagramPrompt: "Собери название главного дела Наталии Сац.", anagramLetters: "Т • Е • А • Т • Р", anagramAnswer: "театр",
...
const important = /(Наталия Ильинична Сац|Наталия Сац|Илья Сац|Анна Щастная|Синяя птица|1917 году|1823|1918 года|1921 году|Сергей Прокофьев|«Петя и волк»|1945 года|21 ноября 1965 года|1979 году|1993 году|Первый детский театр Моссовета|Детский музыкальный театр)/g;
const importantPart = /^(Наталия Ильинична Сац|Наталия Сац|Илья Сац|Анна Щастная|Синяя птица|1917 году|1823|1918 года|1921 году|Сергей Прокофьев|«Петя и волк»|1945 года|21 ноября 1965 года|1979 году|1993 году|Первый детский театр Моссовета|Детский музыкальный театр)$/;
...
export default function SatsRoute({ onBack, onMatching }: { onBack: () => void; onMatching: () => void }) {
  ...
  return <main className="compact highrise-module route-start sats-module">
    ...
    {page === "plan" && <>
      ...
      <div className="part-list">
        {["Семья, детство и Синяя птица", "Первые театры и музыка", "Алма-Ата и Дворец музыки"].map((title, index) => <button className="part-row" key={title} onClick={() => startPart(index)}><span><b>Часть {index + 1}. {title}</b><small>3 учебных экрана → 5 заданий</small></span><span>Открыть →</span></button>)}
    ...
    {page === "lesson" && <>
      <div className="progress"><span>САЦ • ЧАСТЬ {part + 1} ИЗ 3 • ЭКРАН {step % 3 + 1} ИЗ 3</span><div><i style={{ width: `${(step + 1) / 9 * 100}%` }} /></div></div>
      <section className="spread">
        <aside><img src={`./assets/${lessonMedia[step][0]}`} alt={lessonMedia[step][1]} /><small>ИЛЛЮСТРАЦИЯ ИЗ КНИГИ МАРШРУТА</small><p><strong>{lessonMedia[step][1]}</strong></p><div className="part-tag">ЧАСТЬ {part + 1}<br /><b>{["Детство и Синяя птица", "Первые театры", "Дворец музыки"][part]}</b></div></aside>
